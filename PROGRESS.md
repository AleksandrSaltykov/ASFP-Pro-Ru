### 2025-10-09 12:15:00
- Action: Подключил CRM backend для поставщиков (полные реквизиты, банковские счета, контакты), оживил отчёты склада на реальных API аналитики и привёл списки номенклатуры/складов к обновлённым требованиям UI (только название без SKU, адаптивная ширина таблиц).
- Result: справочник поставщиков работает поверх `crm.customer` с миграцией 0004, отчёты `Остатки`, `Оборачиваемость`, `KPI отбора` отображают реальные данные вместо заглушек, таблицы «Номенклатура» и «Склады» помещаются в экран и показывают только жирное название; `go test ./gateway/...` и `pnpm --filter web lint` зелёные.
- Next steps: синхронизировать фронтовый фильтр складов с новыми API и покрыть CRM-конвертеры интеграционными тестами.

### 2025-10-07 09:30:00
- Action: Обновил раздел «Склад» на фронтенде: сохранил состояние левого меню, переработал таблицу категорий и внедрил полноценную страницу поставщиков с локальным fallback и широкой формой редактирования.
- Result: Навигация помнит свёрнутые группы, справочники умещаются в видимую область без горизонтального скролла, поставщиков можно создавать и менять даже без backend; `pnpm lint`, `pnpm typecheck` успешны.
- Next steps: Подключить реальные API для поставщиков и расширить отчётность склада с учётом новых справочников.

### 2025-10-06 11:00:50
- Action: Настроил дев-окружение без CORS-проблем: перевёл фронтенд на прокси Vite (\`/gateway\`, \`/crm\`, \`/wms\`), поправил HTTP-клиент и перезапустил стек/сид.
- Result: запросы к CRM/WMS идут с того же origin (http://localhost:5173), preflight не вызывается; поставщики и прочие справочники загружаются через реальный backend; `pnpm --filter web dev` работает (PID указан в отчёте).
- Next steps: если понадобится prod-сборка, задать абсолютные URL (без прокси) и включить CORS на gateway.

### 2025-10-06 14:40:00
- Action: Привёл справочник поставщиков к новой форме (расширенные реквизиты, несколько контактов и счетов, комментарий «Поставщик»), унифицировал списки склада (доступность, остатки, история, нескончаемые, упаковки, типы операций) под заголовок с кнопкой «Создать»/«Обновить», добавил пункт меню «Резервирование», убрал «Коэффициенты пересчёта».
- Result: Каталог поставщиков хранит полные банковские и контактные данные, страницы склада используют единый шаблон, меню отражает актуальные разделы.
- Next steps: Реализовать удаление и синхронизацию поставщиков с backend, подключить реальные данные по резервированию.

### 2025-10-06 10:26:48
- Action: Добавил полноценный CRUD для категорий и единиц склада, подключил демо-пользователя gateway и убрал офлайн-заглушки.
- Result: в списках категорий/единиц доступны команды Создать/Изменить/Удалить с формами, WMS/CRM отвечают через реальные сервисы под `admin@example.com:admin123`, загрузка клиентов работает; `pnpm --filter web lint` зелёный.
- Next steps: при необходимости расширить редактор коэффициентов пересчёта.

### 2025-10-06 04:14:45
- Action: Переключил фронтенд на работу только с реальным API — fallback-данные теперь активируются лишь при `VITE_ENABLE_OFFLINE=true`.
- Result: по умолчанию запросы номенклатуры/складов падают в ошибку, если бэкенд не поднят; офлайн-режим остаётся доступен через новый флаг. `pnpm --filter web lint` зелёный.
- Next steps: уточнить, нужен ли отдельный индикатор «бэкенд недоступен» для удобства.

### 2025-10-06 04:10:05
- Action: Добавил локальное хранилище для номенклатуры, чтобы изменения сохранялись в офлайн-режиме.
- Result: при работе без backend новые позиции, изменения и удаления переживают перезагрузку страницы; `pnpm --filter web lint` зелёный.
- Next steps: при подключении реального API можно очистить локальные данные или внедрить миграцию.

### 2025-10-06 04:05:04
- Action: Исправил подсветку пунктов в правом боковом меню — теперь ссылки справочников активируются только при точном совпадении маршрута.
- Result: «Номенклатура» больше не подсвечивается при открытии других справочников; `pnpm --filter web lint` зелёный.
- Next steps: проверить остальные разделы (например, складские отчёты), если нужна аналогичная логика.

### 2025-10-06 03:58:12
- Action: Подчистил навигацию справочников и довёл цвета таблиц/подписей до единого стиля, починил падение на «Коэффициентах пересчёта».
- Result: меню «Справочники» разделяет «Категории номенклатуры» и «Единицы измерения», таблицы используют голубые шапки/выделение; ConversionsList больше не выбрасывает runtime-ошибку; `pnpm --filter web lint` зелёный.
- Next steps: при необходимости добавить CRUD для каталога единиц и конверсий.

### 2025-10-06 03:48:04
- Action: Выровнял подписи форм и обновил цвета описаний в списках складских справочников.
- Result: текст заголовков теперь начинается строго над полями, описания элементов читаются в тёмной теме; `pnpm --filter web lint` зелёный.
- Next steps: при необходимости дополнительно настроить цвет подсказок и легенд.

### 2025-10-06 03:40:45
- Action: Привёл формы склада к тёмной теме: сделал подписи полей читабельными и убрал белый фон у инпутов, добавил фокусные состояния.
- Result: во всех редакторах складских справочников заголовки читаются, поля окрашены в тёмный фон; `pnpm --filter web lint` зелёный.
- Next steps: при необходимости доработать подсказки/плейсхолдеры для отдельных полей.

### 2025-10-06 01:36:38
- Action: Реализовал полный CRUD для справочника «Склады»: добавил форму создания/редактирования, поддержку удаления, локальный фолбэк и обращение к API.
- Result: команды Создать/Изменить/Удалить/Обновить работают, данные отправляются через `useCreate/Update/DeleteWarehouseMutation`, при офлайне изменения сохраняются в локальном списке; `pnpm --filter web lint` зелёный.
- Next steps: при необходимости подключить валидацию email/телефона и отдельное уведомление об успешном сохранении.

### 2025-10-06 01:05:30
- Action: Вернул пункт «Номенклатура» в блок складских справочников.
- Result: список «Справочники» снова содержит ссылку на `/warehouse/masters/items`; `pnpm --filter web lint` зелёный.
- Next steps: убедиться, что название совпадает с заголовком соответствующей страницы.

### 2025-10-06 00:37:28
- Action: Перестроил меню склада под новый сценарий — переименовал раздел в «Операции склада» и сократил списки до нужных пунктов.
- Result: операции показывают только Приемку, Перемещение, Комплектовку, Отгрузку клиенту и Инвентаризацию; справочники содержат Категорию единицы измерения, Коэффициенты пересчета, Склады и Поставщиков. `pnpm --filter web lint` зелёный.
- Next steps: при необходимости дополнить маршруты для новых наименований (например, отдельную страницу «Комплектовка»).

### 2025-10-06 00:29:04
- Action: Убрал автраскрытие секций склада — состояние аккордеона теперь всегда начинается со свернутых групп, вне зависимости от активного маршрута.
- Result: при открытии раздела «Склад» все категории закрыты, пользователь сам решает, что раскрыть; `pnpm --filter web lint` остаётся зелёным.
- Next steps: возможно, добавить визуальный маркер активного маршрута на заголовке свернутой группы.

### 2025-10-06 00:24:42
- Action: Перекрасил fallback-загрузчик в фирменный градиент и растянул его на весь экран, чтобы убрать белую вспышку при первом заходе в разделы.
- Result: вместо белого фона при загрузке ленивых страниц показывается плавный тёмный экран «Загрузка...»; `pnpm --filter web lint` зелёный.
- Next steps: при желании добавить индикатор прогресса или спиннер.

### 2025-10-06 00:19:56
- Action: Переделал складское подменю: добавил сворачиваемые группы в правом меню и откладывающееся раскрытие по клику, активный раздел раскрывается автоматически.
- Result: Документы/Справочники/Запасы/Отчёты/Сервис представлены как закрытые аккордеоны с стрелками; `pnpm --filter web lint` проходит.
- Next steps: проверить UX на клавиатуре (focus/space раскрытие) и при навигации между подразделами.

### 2025-10-06 00:13:09
- Action: Сжал оба боковых меню в ревамп-макете, уменьшил отступы и сетку колонок, чтобы навигация занимала меньше горизонтального места.
- Result: основное меню шириной ~230px (72px в свернутом виде), правое подменю ~200px, все подписи умещаются; `pnpm --filter web lint` зелёный.
- Next steps: визуально проверить адаптив на 1280px и ниже, при необходимости подстроить брейкпоинты.

### 2025-10-06 00:09:46
- Action: Спрятал встроенное навигационное меню WMS при включённом ревампе — теперь WarehouseShell не рисует левую панель, когда глобальный сайдбар уже показывает подменю.
- Result: при переходе в «Склад» отображается только одно подменю, линтер остаётся зелёным.
- Next steps: подстроить визуальные отступы складских экранов под новую ширину, если потребуется.

### 2025-10-06 00:04:25
- Action: Добавил алиас `@modules` в tsconfig и Vite для поддержки нового конфига сайдбаров.
- Result: дев-сервер успешно резолвит `@modules/warehouse/...`, линт остаётся зелёным.
- Next steps: перезапустить Vite после обновления конфигурации и убедиться, что дерево модулей загружается без ошибок.

### 2025-10-06 00:02:05
- Action: Перестроил левую навигацию: основное меню теперь автоматически схлопывается до иконок вне главной страницы, добавлен отдельный столбец с подменю активного раздела и вынес конфигурацию модулей.
- Result: при переходе по разделам видно компактную колонку с иконками и подробное меню выбранного модуля; `pnpm --filter web lint` проходит без замечаний.
- Next steps: дополнить конфигурацию подменю для остальных модулей и проверить UX на компактных экранах.

### 2025-10-03 21:45:00
- Action: Расширил мастер-данные WMS: добавил UI для единиц, атрибутов и карточек изделий, связал формы с backend CRUD и обновил MSW моки.
- Result: страницы `/warehouse/masters/items`, `/warehouse/masters/items/units`, `/warehouse/masters/items/attributes` работают, новые API эндпоинты доступны через WMS; lint и go test ./... (кроме известных smoke) проходят.
- Next steps: дополнить сценарии с товарами (CRUD/валидация) и синхронизировать smoke с обновленным gateway.

### 2025-10-03 21:17:44
- Action: Собрал первый раздел WMS: добавил страницу 'Категории номенклатуры' с древовидным отображением, формами создания/редактирования/удаления и валидацией.
- Result: crud по каталогу 'category' работает через gateway, UI обновляет иерархию на лету; msw выдаёт мок-ендпоинты для catalog/*, lint проходит.
- Next steps: расширить справочник единиц/атрибутов и привязку к товарам, затем протянуть e2e на Playwright.

### 2025-10-03 20:36:36
- Action: Добавил подсветку пунктов выпадающего меню по наведению и переписал верхнюю панель на grid/hover state.
- Result: элементы дропдаунов сразу меняют цвет при hover, меню по-прежнему стабильно и занимает две ровные строки; unit-тесты проходят.
- Next steps: проверить работу на тач-устройствах/клавиатуре.

### 2025-10-03 20:30:44
- Action: Убрал разрыв между кнопкой и выпадающим меню: wrapper получил нижний отступ, dropdown теперь позиционируется по top: 100%.
- Result: при переходе курсора в подпункты выпадашка больше не закрывается; unit-тесты остаются зелёными.
- Next steps: проверить работу на touch-устройствах и при большом масштабе.

### 2025-10-03 20:16:50
- Action: Фиксанул dropdown для разделов "Запасы", "Отчёты", "Настройки": выключил преждевременное закрытие при наведении и разложил верхнее меню решёткой в две строки.
- Result: меню держится под курсором во всех разделах, всплывающие списки работают одинаково, верхняя панель заполнена ровно по сетке; unit-тесты зелёные.
- Next steps: подумать о поддержке touch и клавиатурной навигации, если потребуется.

### 2025-10-03 20:10:32
- Action: Настроил поведение dropdown верхнего меню: оно остаётся открытым при переходе к подпунктам, закрывается только при уходе за границы и поддерживает Esc.
- Result: пользователь может спокойно перемещаться по выпадающим пунктам без исчезновения, прозрачность и подсветка сохранены; unit-тесты проходят.
- Next steps: проверить UX на touch-устройствах и добавить close по Esc на остальные панели, если потребуется.

### 2025-10-03 19:56:17
- Action: Доработал верхнее меню склада: исключил двойной клик, оставил dropdown открытым при навигации мышью, сделал фон непрозрачным и убрал подчёркивания.
- Result: меню открывается с первого раза, удерживается при переходе по подпунктам, отображается без прозрачности/подчёркиваний; unit-тесты возвращаются зелёными.
- Next steps: рассмотреть поддержку закрытия по Esc и адаптив для мобильных.

### 2025-10-03 19:42:47
- Action: Исправил поведение верхнего меню склада: убрал навигацию при первом клике, добавил закрытие по клику вне меню, сделал всплывающие списки непрозрачными и убрал подчёркивания.
- Result: первый клик разворачивает подпункты, dropdown остаётся открытым при наведении и закрывается только вручную; фон плотный, стили без подчёркиваний; unit-тесты зелёные.
- Next steps: подумать над улучшением адаптива и закрытием по Esc.

### 2025-10-03 19:36:33
- Action: Привёл верхнее меню склада к более лёгкому виду: убрал номера из подписей, сделал текстовые триггеры без рамок и оставил dropdown по клику.
- Result: навигация над модулем выглядит компактно, подпункты раскрываются из слов, внешний вид не перебивает контент страниц; unit-тесты после правок проходят.
- Next steps: доработать закрытие выпадашек по клику вне области и адаптив под планшеты.

### 2025-10-03 19:29:53
- Action: Обновил навигацию модуля склада: заменил боковую колонку на верхнее горизонтальное меню с выпадающими пунктами и донастроил vitest на проверку только unit-тестов.
- Result: раздел /warehouse стал легче визуально — верхнее меню открывает подпункты по клику, при смене роута сворачивается автоматически; unit-тесты проходят, e2e остаются под Playwright.
- Next steps: доработать стили (адаптив, закрытие по клику вне области) и согласовать финальный UX с командой.

### 2025-10-03 18:50:19
- Action: Перестроил фронтовой модуль склада: вынес старый код в архив, настроил новую вложенную навигацию /warehouse/* и собрал страницы 8.1–8.4 на основе штатных компонентов и мок-сервисов.
- Result: разделы `Остатки`, `Доступность`, `Нескончаемые остатки`, `История движений` работают через новые таблицы/фильтры, маршруты старого `/wms/inventory` редиректят на `/warehouse/stock/balances`, остальные подразделы отображают заглушки.
- Next steps: Дополнить Endless-политику интеграцией с backend после появления API, согласовать UI copy/tables и добить упавшие vitest-тесты (OrderStepper).

### 2025-10-03 15:39:16
- Action: Прогнал миграции WMS (0001-0005) и восстановил таблицы оборудования/историй после пересоздания стенда.
- Result: API `/api/v1/master-data/warehouses/:id` снова отдаёт зоны/ячейки/оборудование без 500; фронтенд WMS прогружает данные.
- Next steps: хранить `DATABASE_URL` для повторных `make migrate-wms`, при необходимости сделать миграцию 0005 идемпотентной.

### 2025-10-01 15:45:00
- Action: Подключил проверку прав во всех маршрутах gateway: PermissionGuard, обновлённые роли/сид, фронтенд-админка (оргструктура, токены) и Playwright-сценарии.
- Result: CRUD WMS/CRM/MES/Montage/Docs/BPM/Analytics требуют разрешений из `core.role_permissions`; директор имеет полный доступ, узкие роли ограничены скоупами. В UI доступны страницы управления правами, e2e проверяет их доступность.
- Next steps: пронести `Subject` в доменные сервисы для фильтрации данных (склады, сделки), связать роли с UI (скрытие меню) и расширить smoke e2e тестами для RBAC.

### 2025-10-01 09:10:00
- Action: Добавил расширенный RBAC: оргструктуру, матрицу прав, API токены. Обновил core сервис/хендлеры, миграции, seed и smoke-тест.
- Result: core.org_units/role_permissions/api_tokens доступны через новые эндпоинты, gateway обрабатывает CRUD, smoke сценарий проверяет создание юнита/матрицы/токенов, OpenAPI отражает контракт.
- Next steps: Включить проверку прав в сервисах (gateway middleware), связать роли с орг-юнитами на фронте, дополнить e2e авторизацией.

### 2025-09-30 13:20:00
- Action: Синхронизировал roadmap/PROGRESS, задокументировал реализованные доменные API, обновил модульные README, фронтенд-гайд и описание тестов.
- Result: Фаза 2 отмечена как завершённая по CRUD/seed/test; roadmap отражает фокус на RBAC, расширенном аудите и BI; README модулей описывают доступные endpoints и сиды; tests/README содержит справку по smoke.
- Next steps: Запустить спринт по RBAC/permission matrix, усилить аудит и события, подготовить BI-витрины и e2e/Playwright сценарии для ключевых потоков.

### 2025-09-30 12:05:00
- Action: Настроил ClickHouse сиды и реализовал минимальный Analytics API (reports/exports) в сервисе и через gateway, задокументировал OpenAPI и добавил smoke-сценарий.
- Result: `/api/v1/analytics/reports/*` и `/api/v1/analytics/exports/*` доступны через gateway, демо-данные подгружаются в ClickHouse, smoke `gateway_analytics_reports_exports` проходит.
- Next steps: Подготовить e2e BI сценарий и расширить отчётность при необходимости.

### 2025-09-30 10:38:41
- Action: Реализовал BPM minimal API (processes/forms/tasks) с расширенной схемой (`assignment_rule`, `escalation`), обновил миграции/seed, OpenAPI сервисов и подключил сценарий в gateway + smoke.
- Result: миграция 0001 дополнилась новыми таблицами, `deploy/.env.example` и gateway OpenAPI отражают BPM, placeholder в smoke заменён CRUD сценарием; `go test` (кроме smoke) проходит локально.
- Next steps: поднять docker-compose стенд и прогнать `make smoke` с новым BPM сценарием, затем перейти к подготовке minimal API для Analytics.

### 2025-09-30 06:41:22
- Action: Реализовал Montage minimal API (бригады/транспорт/задачи) в отдельном сервисе и через gateway, добавил схему БД, миграции, OpenAPI и Docker-образ.
- Result: docker compose пересобирает сервисы, make seed наполняет новые таблицы, smoke сценарий для montage готов (требуется прогон после ребилда).
- Next steps: Запустить make smoke после обновления окружения и приступить к Docs minimal API.

### 2025-09-30 05:44:51
- Action: Реализовал minimal MES API (work-centers/operations/routes) в отдельном сервисе и через gateway: схема Postgres, репозитории/сервисы, HTTP-хендлеры, Docker и OpenAPI.
- Result: docker compose собрал новый mes, миграции/seed прогнаны, go test ./..., make smoke подтверждает сценарий mes_minimal_api.
- Next steps: Перейти к подготовке minimal API для Montage и расширить smoke по аналогии.

### 2025-09-30 01:07:32
- Action: Привёл CRM/WMS/gateway код к gofmt, связал CRM сервис с gateway, поправил updateDeal/UpdateCustomer без несуществующего updated_at, убрал NULLIF для createdBy и пересобрал контейнеры перед make seed/check-demo/smoke.
- Result: CRM minimal API отдаёт данные через gateway (/api/v1/crm/*), smoke gateway_crm_customer_deal_crud и go test ./... проходят, CRM сервис корректно обрабатывает пустой created_by.
- Next steps: Приступить к реализации minimal API для MES и добавить для него smoke сценарии.

### 2025-09-29 23:46:00
- Action: Реализовал minimal Core API (users/roles) в gateway: добавлены репозиторий/сервис, REST-обработчики и аудит.
- Result: /api/v1/users и /api/v1/roles доступны через gateway, сиды + smoke/e2e проверяют демо-данные; placeholders готовы к снятию после следующих доменов.
- Next steps: Перейти к реализации WMS minimal API, затем CRM, и включить новые smoke/e2e проверки без skip.

### 2025-09-29 23:26:36
- Action: Добавил seed-файлы для MES/Montage/Docs/BPM и демо-вставки для ClickHouse, обновил smoke/e2e под CRM/WMS и задал плейсхолдеры для будущих доменов.
- Result: make seed покрывает новые домены (с безопасным пропуском, если таблиц ещё нет), smoke проверяет CRM demo и содержит placeholders, Playwright отслеживает CRM и будущие разделы.
- Next steps: Реализовать минимальные API по приоритету (Core -> WMS -> CRM) и включить новые smoke/e2e сценарии без skip.

### 2025-09-29 23:00:05
- Action: Добавил таблицу minimal API/seed/test требований для всех доменов Фазы 2 и отметился в чеклисте roadmap.
- Result: Понятно, какие эндпоинты, сиды и типы тестов нужны для каждого сервиса перед разработкой; чеклист дополнен отдельным пунктом.
- Next steps: Перейти к плану реализации minimal API (приоритизация доменов) и подготовке сидов/тестов.

### 2025-09-29 22:52:30
- Action: Зафиксировал обязательные события, аудит и Swagger-артефакты для доменов Фазы 2 (таблица в docs/roadmap.md).
- Result: Чеклист Фазы 2 пополнен деталями по событиям/аудиту; понятно, какие топики и действия нужны для каждого домена.
- Next steps: Сформировать план minimal API и обновление сидов/тестов под выбранные домены.

### 2025-09-29 22:44:00
- Action: Составил матрицу доменов и справочников (core/CRM/WMS/MES/монтаж/BPM/docs/analytics) и добавил её в docs/roadmap.md.
- Result: Фаза 2 имеет зафиксированный объем данных и зависимостей; можно переходить к описанию событий, аудита и Swagger для доменов.
- Next steps: Описать требования к обязательным событиям/аудиту/Swagger для сервисных пластов.

### 2025-09-29 22:37:12
- Action: Обновил roadmap — зафиксирован переход к Фазе 2 и новый чекпоинт (матрица доменов, события/Swagger, план API).
- Result: Текущая позиция отражает завершение Фазы 1, чекпоинты сфокусированы на сервисных пластах.
- Next steps: Подготовить матрицу доменов ↔ справочники и зафиксировать её в docs/roadmap.md.

### 2025-09-29 22:26:33
- Action: Подготовил how-to по расширению справочников без миграций и зафиксировал правила тестового покрытия (docs/masterdata/how-to-extend-directories.md).
- Result: Roadmap обновлён, ссылки на документацию добавлены в Фазу 1; динамический контракт ссылается на обновлённый процесс.
- Next steps: Проверить необходимость дополнительной автоматизации для `make smoke` при появлении новых справочников.

### 2025-09-29 22:15:38
- Action: Описал контракт динамических атрибутов и порядок валидации/значений для WMS master data (docs/masterdata/dynamic-attributes.md).
- Result: Зафиксированы допустимые типы, поведение сервисного слоя и форматы API; ссылки в roadmap обновлены.
- Next steps: Подготовить how-to по добавлению новых справочников/атрибутов без миграций и зафиксировать требования к тестовому покрытию.

### 2025-09-29 19:36:39
- Action: Удалил BOM из core/crm миграций, переписал seed на jsonb_build_object и проверил up/down для всех модулей.
- Result: `make migrate-core|crm|wms` и соответствующие `*-down`/`*_DOWN_TO=0` проходят на чистой БД; CI теперь гоняет полный цикл up/down/up для всех модулей.
- Next steps: Подумать об автоматическом `make seed` в CI после откатов и оценить необходимость очистки связанных сервисов при down-to 0.

### 2025-09-29 19:10:45
- Action: Добавил down-таргеты для core/crm, задокументировал команды и встроил прогон `make migrate-wms` + откаты в CI (postgres-only этап).
- Result: Makefile поддерживает `migrate-core-down`/`migrate-crm-down`, README описывает откаты, а workflow CI прогоняет последовательность up/down/up `make migrate-wms`.
- Next steps: Решить, нужны ли аналогичные проверки для core/crm в CI и автоматизировать восстановление демо-данных после полных откатов.

### 2025-09-29 19:06:25
- Action: Добавил таргет `migrate-wms-down` с параметром `WMS_DOWN_TO`, проверил откат/повторный запуск через Makefile.
- Result: `make migrate-wms-down` откатывает последнюю миграцию, `WMS_DOWN_TO=0 make migrate-wms-down` сбрасывает схему, а `make migrate-wms` повторно поднимает версии 0001–0004.
- Next steps: Убедиться, что после полного отката seed наполняет демо-данные, и продумать аналогичные проверки для других модулей.

### 2025-09-29 19:00:50
- Action: Добавил fallback на `go run goose` в Makefile, обновил README по миграциям и прогнал `make migrate-wms` на чистой базе.
- Result: Таргет `make migrate-wms` автоматически подтягивает goose при отсутствии бинаря и успешно применяет миграции 0001–0004; README содержит инструкцию по `DATABASE_URL`.
- Next steps: Подумать над аналогичным fallback для core/CRM миграций в CI и при необходимости добавить `migrate-wms-down` для отката.

### 2025-09-29 18:54:54
- Action: Перекодировал миграцию 0004_seed_dynamic_masterdata.sql в UTF-8, поправил SQL (корневой path) и применил сид через goose.
- Result: В wms.catalog_node появились категории SIGNAGE/PRINT, созданы шаблоны атрибутов и демонстрационный товар DEMO-SIGN-001; команда goose up завершилась успешно.
- Next steps: Обновить Makefile/README для автоматического запуска goose (включая сид 0004) и прогнать make migrate-wms на чистом стенде для проверки.

### 2025-09-29 18:36:25
- Action: Обновил WMS backend, обработчики репозитория и применил миграции 0001–0003 через goose (локально).
- Result: `/api/v1/master-data/warehouses/{id}` отвечает 200, фронтенд раздел «Склад» загружает данные без ошибки; схема WMS синхронизирована с актуальными таблицами.
- Next steps: Перекодировать `modules/wms/migrations/0004_seed_dynamic_masterdata.sql` в UTF-8 и повторить сид; убедиться, что `make migrate-wms` работает без go run на CI.

### 2025-09-29 17:36:56
- Action: Подключил core.audit_log к analytics (consumer+API), добавил документирование и UI-страницу журнала.
- Result: события DealCreated фиксируются в Postgres, отчёты/запросы gateway отражаются в `/api/v1/audit`, веб-клиент показывает журнал для админов.
- Next steps: добавить фильтры по дате/типам событий и расширить покрытие других модулей при появлении новых use-case.

### 2025-09-28 20:55:48
- Action: Доработал динамические мастер-данные WMS (обработка NULL category_path, обновлённые smoke-сценарии, документация).
- Result: `go test ./...` зелёный, контейнер wms пересобран через docker compose, CRUD сценарии master data завершаются без 500.
- Next steps: Зафиксировать изменения в feature/dynamic-masterdata и вернуться к переносу динамических атрибутов в CRM.

﻿### 2025-09-22 21:17:38
- Action: попытка docker compose ... up --build -d
- Result: build failed. go.mod требует go >= 1.23.0, а базовые образы используют golang:1.22-alpine > сборка gateway/crm/wms/analytics остановилась.
- Next steps: обновить Dockerfile сервисов на golang:1.24-alpine (или задать GOTOOLCHAIN=auto) и повторить сборку.

### 2025-09-22 21:22:10
- Action: docker compose up --build -d (after switching to golang:1.24-alpine)
- Result: сервисы собраны, но контейнер ceph упал — в образе quay.io/ceph/ceph:v18 нет бинаря demo, указанный entrypoint demo --rgw не найден.
- Next steps: заменить образ на quay.io/ceph/ceph:v18 с корректным entrypoint либо использовать quay.io/ceph/demo:latest (исторический демо-образ) и обновить команду/окружение.

### 2025-09-22 21:40:40
- Action: docker compose up --build -d после замены Ceph на quay.io/ceph/demo:latest
- Result: все контейнеры собраны и стартовали (redis, postgres, tarantool, clickhouse, ceph, gateway, crm, wms, analytics, nginx).
- Next steps: проверить health-checkи сервисов и убедиться, что Ceph RGW отвечает по демо-портам.

### 2025-09-22 21:59:11
- Action: инициализация git-репозитория, commit и push.
- Result: ветка main опубликована в https://github.com/AleksandrSaltykov/ASFP-Pro-Ru.
- Next steps: после проверки стенда добавить health-checkи и тесты.

### 2025-09-22 22:29:19
- Action: проверка сервисов /health.
- Result: gateway/crm/wms не стартуют из-за ошибки S3 — Ceph demo контейнер падает (требует корректной MON_IP/NETWORK конфигурации). Tarantool фиксирован, Ceph всё ещё в статусе Exited.
- Next steps: настроить ceph-demo (указать CEPH_DEMO_BUCKET, CEPH_PUBLIC_NETWORK, CEPH_CLUSTER_NETWORK, корректный MON_IP/NETWORK_AUTO_DETECT) либо временно заменить на MinIO для dev, затем повторить health-check.

### 2025-09-22 23:16:53
- Action: заменили demo Ceph на MinIO (S3-совместимый стенд), добавили fallback для OpenAPI и пересобрали сервисы.
- Result: MinIO запущен на :7480/:9001, gateway/crm/wms отдают 200 на /health.
- Next steps: уточнить в документации, что для продакшена требуется Ceph RGW, и при необходимости добавить healthcheck MinIO.

### 2025-09-22 23:43:10
- Action: добавлен GitHub Actions workflow (gofmt + go test).
- Result: любой push/PR на main гоняет базовую статическую и тестовую проверку (GOTOOLCHAIN=auto).
- Next steps: при появлении smoke/интеграционных тестов можно расширить job.

### 2025-09-22 23:47:18
- Action: go test ./... и smoke-тесты (ручная выгрузка) выполнены локально.
- Result: все пакеты проходят тесты, MinIO принимает загрузку.
- Next steps: при необходимости расширить unit-тесты CRM/WMS.

### 2025-09-23 00:04:44
- Action: добавлен скрипт scripts/minio-reset.sh и расширены smoke-тесты (проверка OpenAPI + upload).
- Result: make smoke теперь читает /openapi.json; скрипт пересоздает MinIO bucket через minio/mc.
- Next steps: при необходимости интегрировать smoke в CI и автоматизировать вызов minio-reset перед тестами.

### 2025-09-23 00:10:54
- Action: интегрировал make smoke в CI (docker compose up -> smoke -> down).
- Result: GitHub Actions теперь поднимает весь стек, сбрасывает MinIO и гоняет smoke-тесты автоматически.
- Next steps: контролировать длительность job и при необходимости кэшировать docker build.

### 2025-09-24 00:30:00
- Action: ограничены ресурсы docker-compose (mem_limit/cpus), автоматизирован вызов mkcert в Makefile и расширены smoke-тесты (артефакты, HTTPS-ветка). README дополнен инструкциями по сертификатам и лимитам.
- Result: `make up` генерирует локальные сертификаты до старта стенда (поддержан `SKIP_MKCERT`), smoke складывает логи в `tests/smoke/artifacts` и умеет ходить по HTTPS через nginx, документация и конфигурация синхронизированы; gateway отдаёт стартовую страницу Control Center.
- Next steps: прогнать `mkcert -install` и `make up` на чистой машине с `SMOKE_GATEWAY_HTTPS_URL=https://localhost:8443 make smoke`; в CI замерить длительность smoke job и добавить кеширование сборок (Go модули и docker build), если это даст выигрыш.

### 2025-09-24 00:55:00
- Action: добавлен кеш Go модулей/артефактов в CI и прогрев docker buildx через bake + локальный кэш перед docker compose up.
- Result: workflow сохраняет `~/go/pkg/mod`, `~/.cache/go-build` и buildx слои (`cache-to/cache-from`), что сокращает время lint/test и сборки образов в smoke job; compose больше не делает `--build`, используя предварительно собранные образы.
- Next steps: проверить, насколько сократилось время GitHub Actions; при необходимости перенести build-этап на `docker/build-push-action` с `scope` per-service и/или добавить `cache-to=type=gha` для более агрессивного шеринга.

### 2025-09-24 13:22:30
- Action: инициализировали фронтенд-монорепозиторий (pnpm workspace) и scaffold SPA на React/Vite.
- Result: добавлены скрипты, базовые провайдеры (Redux Toolkit, React Query), роутинг, макеты страниц и MSW; тесты Vitest проходят.
- Next steps: подключить дизайн-систему (Ant Design fork или свой UI-kit), сгенерировать API-клиенты из OpenAPI и реализовать авторизацию/канбан CRM.
### 2025-09-24 19:25:00
- Action: Определили необходимость фиксировать прогресс после каждого шага и запросили детальный анализ брендбука (логотипы, палитры, шрифты).
- Result: Брендовые ассеты найдены в d:\работа\asfp soft\Брендбук\...; приступили к расшифровке SVG и палитр.
- Next steps: 1) Проанализировать палитру, типографику, иконки; 2) Обновить фронтенд под стиль брендбука; 3) Вести записи в PROGRESS.md после каждого шага.
### 2025-09-24 19:45:00
- Action: Проанализировали брендовые SVG (цвета #2962FF, #131A2D, белый) и обновили фронтенд-оболочку: ввели theme.ts, пересобрали навигацию и layout, добавили загрузку шрифта Manrope.
- Result: Главный каркас отражает фирменные цвета, есть универсальная тема и стилизованные боковые/верхние меню; eslint-запуск не завершён (в проекте отсутствует eslint.config.js).
- Next steps: 1) Подтвердить фирменный шрифт по брендбуку или заменить; 2) Собрать библиотеку легких иконок в фирменном стиле; 3) Довести до соответствия контентные страницы.
### 2025-09-24 19:55:00
- Action: Уточнили брендовые шрифты (основной Onest, декоративный Inter) и обновили тему/верстку: подключили Google Fonts, обновили theme.ts, MainLayout и навигацию.
- Result: Фронтенд использует Onest для базового текста и Inter 28pt как акцентный, шапка и сайдбар отражают фирменную типографику.
- Next steps: Внедрить новый стиль на внутренних страницах и адаптировать иконки под брендовые гайды.
### 2025-09-24 20:05:00
- Action: Применили фирменную тему на страницах CRM, WMS и Files: карточные описания с Onest/Inter, блоки подсветки, фирменные градиенты.
- Result: Внутренние страницы визуально совпадают с брендбуком, готовы для интеграции виджетов.
- Next steps: Собрать фирменные иконки и подключить их к сайдбару и контентным блокам.
### 2025-09-24 20:10:00
- Action: Запустили фронтенд (Vite, порт 5173) для визуальной проверки обновлённого UI.
- Result: Интерфейс доступен по http://localhost:5173/, тема и страницы выглядят согласно брендбуку.
- Next steps: Сконвертировать брендовые иконки и встроить их в приложение.
### 2025-09-24 20:18:00
- Action: Подготовили набор фирменных иконок (overview/crm/wms/files/system) и внедрили их в боковое меню; иконки построены в цветах #2962FF/#131A2D/#E6EDFF.
- Result: Навигация теперь передаёт смысловые акценты брендбука через векторные пиктограммы; код лежит в @shared/ui/icons.
- Next steps: Завершить настройку lint (eslint.config) и отключить временные devserver артефакты перед коммитом.
### 2025-09-24 20:25:00
- Action: Перерисовали иконки под стиль образца (grid, clipboard, куб, папка, gear).
- Result: Иконки минималистичны, stroke 1.7, подчёркивают смысл пунктов меню.
- Next steps: Проверить визуально и утвердить набор.

### 2025-09-27 22:45:00
- Action: git reset --hard origin/main && git clean -fd; сформирован план по универсальным каталогам/атрибутам.
- Result: рабочее дерево приведено к чистому состоянию; зафиксирован roadmap реализации динамического WMS.
- Next steps: 1) подготовить миграции для catalog_node, attribute_templates/values и связей с переносом текущих данных; 2) обновить репозитории/сервисы для generic-справочников, динамических атрибутов и catalog_links; 3) расширить HTTP-handlers и тесты, обеспечить выдачу core+attributes; 4) адаптировать фронтенд (API-клиент, формы, UI) под динамические шаблоны и связи.

### 2025-09-27 23:25:00
- Action: Реализованы сущности, репозитории и сервисы для универсальных каталогов и динамических атрибутов WMS.
- Result: Добавлены entity/catalog.go, entity/attribute.go, entity/item.go; миграция 0003_dynamic_catalogs.sql и embed.go; repository/catalog_repository.go и расширения masterdata_service.go; сборка go test ./... проходит (smoke падал как раньше).
- Next steps: Расширить HTTP-слой (handlers, OpenAPI) для работы с generic-справочниками и динамическими атрибутами и обновить тесты.

### 2025-09-27 23:56:00
- Action: Расширены HTTP-хендлеры WMS, добавлены generic/catalog/item endpoints и тесты конвертации запросов; обновлён OpenAPI.
- Result: masterdata_handler.go обслуживает справочники, товары, связи; добавлен masterdata_handler_test.go; OpenAPI описывает новые маршруты; go test ./... (кроме исторического smoke) проходит.
- Next steps: Обновить фронтенд API/формы для работы с динамическими справочниками и атрибутами, связать с новыми endpoint 'ами. 

### 2025-09-28 12:15:00
- Action: Обновил README под актуальный dev-стек (MinIO по умолчанию, статус smoke/CI), довёл фронтенд lint/tsconfig до рабочего состояния и подключил React Query-клиенты к новым WMS endpoint (каталоги, атрибуты, товары).
- Result: README отражает MinIO-first сценарий и предупреждает о нестабильном smoke, `pnpm lint`/`pnpm build` проходят без js-артефактов (tsconfig `noEmit`), `CatalogManager`/`ItemManager` используют API (`useCatalogNodesQuery`, `useItemsQuery`, `useReplaceCatalogLinksMutation`).
- Next steps: интегрировать формы с реальными бэкенд-токенами и правами, покрыть новые хэндлеры e2e-тестами и зафиксировать smoke-фиксы после стабилизации CI.
### 2025-10-05 02:36:00
- Action: Пересобрал фронтовый модуль «Склад»: перенёс код в `/modules/warehouse`, добавил каркас 1С (LeftNav + CommandBar + Filters + ListForm + StatusBar), реализовал раздел 8 (Остатки/Доступность/Нескончаемые/История) с фирменной палитрой и свертывающимся меню, переписал роуты на новый layout и восстановил глобальную тему.
- Result: `/warehouse/*` использует новый shell с навигацией-аккордеоном и страницами в синих фирменных цветах; placeholder-компоненты подключены вместо ещё не перенесённых журналов; `pnpm lint` и `pnpm test -- --run tests/stock` зелёные; старый модуль сохранён в `apps/web/src/pages/warehouse-backup-20251005-0112`.
- Next steps: интегрировать реальные API вместо заглушек (`services/*.ts`), перенести справочники и документы из бэка и покрыть end-to-end-сценарии Playwright.
### 2025-10-09 14:30:00
- Action: Завершил отрисовку раздела «Справочники» в модуле склада: добавил обзорную страницу с виджетами, привязал поставщиков к реальному CRM с Basic Auth, подчистил таблицы (иконки вместо кнопок, переносы строк, компактные высоты), выставил сайдбар свернутым.
- Result: главная страница справочников показывает живые данные и индикаторы статусов, списки номенклатуры/поставщиков/складов читаемые на любых разрешениях, preflight не блокируется CORS; `pnpm --filter web lint` и `go test ./gateway/...` проходят.
- Next steps: при необходимости добавить графики в обзор и покрыть CRM-интеграцию интеграционными тестами.
