import { WarehouseMasterData } from './components/WarehouseMasterData';
...
const fullWidthBlockStyle: CSSProperties = {
  gridColumn: '1 / -1',
  display: 'block'
};
    ...
    tabs: [
      {
        ...
        summary:
          'Многоскладовая архитектура, адресная система, зонирование, буферы и регистры оборудования. Полный цикл создания и сопровождения карточек объектов хранения.',
        blocks: [
          {
            title: 'Карточки складов',
            description: 'Паспорт площадки: адрес, кадастр, ответственные, режимы работы, классы складов, допустимые операции.',
            items: [
              'Мастер создания склада с валидацией обязательных реквизитов и чек-листами безопасности',
              'Привязка к юридическому лицу, договорам аренды/собственности, регламентам охраны труда',
              'Настройка режимов работы (смены, окна приёмки/отгрузки, "тихие" часы, ночная логистика)',
              'Схема инфраструктуры: ворота, рампы, зоны парковки, энергомощности, инженерные системы'
            ]
          },
          {
            title: 'Зональная топология',
            description: 'Структура складской топологии: функциональные зоны, буферы, участки спецхранения.',
            items: [
              'Конструктор зон (приёмка, хранение, подбор, экспедиция, кросс-док, WIP, карантин)',
              'Параметры контроля: температура, влажность, санитарные нормы, классы опасности',
              'Буферы "в пути", лабораторные и таможенные зоны, стыковка с производственными цехами',
              'Визуализация планов в 2D/3D, интеграция с BIM и планами эвакуации'
            ]
          },
          {
            title: 'Адресная система и slotting',
            description: 'Подсистема управления ячейками, стеллажами, уровнями и правилами размещения.',
            items: [
              'Иерархия: склад → зона → коридор → стеллаж → уровень → ячейка → ярус',
              'Типы ячеек (паллетные, мелкоячеечные, гравитационные, холодильные, опасные грузы)',
              'Нормативы грузоподъемности, габаритов, допустимых товаров и совместимости',
              'Правила slotting: ABC/XYZ, скорость оборота, совместимость, ограничения по опасности и ADR'
            ]
          },
          {
            title: 'Жизненный цикл и формы',
            description: 'Создание, согласование, изменение и закрытие карточек складов и ячеек.',
            items: [
              'Мастера создания: ручной ввод, bulk-импорт, генерация по шаблонам или CAD',
              'Workflow: инициатор → технолог → охрана труда → ИТ → эксплуатация',
              'Журнал изменений, версии схем, автоматические уведомления задействованных служб',
              'Процедуры консолидации, разделения, временного закрытия ячеек (ремонт, дезинфекция)'
            ]
          },
          {
            title: 'Оборудование и привязки',
            description: 'Регистры техники, сенсоров, весового оборудования и автоматизированных линий.',
            items: [
              'Привязка погрузчиков, AGV, шаттлов, конвейеров к зонам, ячейкам и сменам',
              'Учёт датчиков IoT (температура, вибрации, нагрузка) с alert-порогами и SLA',
              'Графики ТО, ответственные механики, интеграция с CMMS и системой заявок',
              'Интерфейсы с WCS/АСУТП, мониторинг доступности и резервирование ресурсов'
            ]
          },
          {
            title: 'Интеграции и аналитика',
            description: 'Обмен с ERP, BIM, CAD, системами ПБ и аналитические витрины.',
            items: [
              'Двусторонняя синхронизация с ERP и бухгалтерией (создание/закрытие складов, атрибуты, лимиты)',
              'Импорт/экспорт топологии в CAD/BIM (Revit, AutoCAD) и системы пожарной безопасности',
              'Отчёты по заполненности, эффективности slotting, коэффициенту использования объема',
              'API для цифровых двойников, AR-навигации и мобильных приложений операторов'
            ]
          }
        ]
      },
  ...
  return (
    <section style={sectionStyle}>
      ...
      <div style={blocksGridStyle}>
        {activeTab.id === 'warehouses' ? (
          <div style={fullWidthBlockStyle}>
            <WarehouseMasterData />
          </div>
        ) : null}
