...
- Боковое меню склада запоминает состояние свёрнутых групп в `localStorage`, поэтому после перезагрузки структура остаётся в том виде, в котором её оставил пользователь.
- В справочниках реализованы полноценные CRUD-интерфейсы:
  - **Категории номенклатуры** с древовидным отображением, автогенерацией кодов и формой редактирования в несколько вкладок.
  - **Номенклатура** — поддержка альтернативных единиц и коэффициентов пересчёта, автоматическое создание SKU; список отображает только человекочитаемое название позиция без технического кода.
  - **Единицы измерения** и **Склады** — списки с фильтрами, формами и локальным fallback.
  - **Поставщики** — таблица с реквизитами (ИНН), контактами и адресами, редактор банков и контактных лиц; при недоступном backend данные сохраняются локально.
