...
- Клиентское SPA (`apps/web`) использует верхнее меню склада по адресу `/warehouse/*`: разделы `stock/balances`, `stock/availability`, `stock/endless`, `stock/history` доступны сразу после редиректа со старого `/wms/inventory`. Остальные пункты структуры работают как заглушки до реализации бекенда.
- Unit-тесты фронта выполняются через `pnpm --filter web test -- --run`, Playwright-сценарии остаются в `apps/web/e2e`.

 — API-шлюз, авторизация, загрузка файлов, проксирование модулей и core API (`/api/v1/users`, `/api/v1/roles`).
