...
- Для синхронизации дампов с GitHub добавлен скрипт `tools/git-backup-sync.sh` (обёртка доступна через `make backup-sync`). Он снимает свежий `pg_dump`, добавляет файл в `backups/` и делает коммит. Перед запуском убедитесь, что у вас настроен Git-пользователь. Для автоматической отправки в удалённый репозиторий задайте переменные окружения:
  - `GIT_BACKUP_PUSH=1` — включить `git push` после коммита;
  - `GIT_BACKUP_REMOTE` (по умолчанию `origin`) и `GIT_BACKUP_BRANCH` (по умолчанию `main`);
  - `GIT_BACKUP_KEEP=N` — оставить только N последних дампов в каталоге (старые будут удалены после успешного коммита).
  Секреты (PAT/SSH-ключи) задаются стандартным способом: через `git credential helper`, SSH-агент или переменные среды оболочки.
